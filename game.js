...
function displayInventory() {
    ...
    for (let i = 0; i < MAX_BACKPACK_SIZE; i++) {
        ...
        if (i < player.backpack.length) {
            ...
            // Use specific image or category default or fallback
            const imgSrc = item.imageSrc || imagePaths[category]?.[tier] || imagePaths[category]?.default || 'https://placehold.co/40x40/555/eee?text=???'; // Corrected logic
...
function generateStatsHTML(item, compareToItem = null) {
    ...
    if (!hasStats && item?.category !== 'consumable') { // Don't show "No Stats" for consumables
         statsHTML += "<li>No Stats</li>";
    }
    return statsHTML;
}
